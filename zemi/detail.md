...
**役割**: `window.matchMedia`をReact stateとして購読する小さな共通フック(新規、Codexレビュー指摘への対応で追加)。コンテナ要素の実測サイズ(`ResizeObserver`等)ではなく、常にビューポート基準でCSSのブレークポイントと同じ判定をしたい箇所(デスクトップ/モバイル分岐)で使う。

**主要な型・関数**:
- `DESKTOP_MEDIA_QUERY = '(min-width: 1024px)'` — TailwindのCSSブレークポイント`lg`(1024px)と完全に一致させるためのメディアクエリ文字列定数。デスクトップ判定を行う全箇所がこの定数を共有し、別の基準値を独自に持たないようにする。
- `useMediaQuery(query: string): boolean` — `useState(() => window.matchMedia(query).matches)`で初期値を同期的に取得(SSRは行わないため`window`に直接アクセス)。`useEffect`で`MediaQueryList`の`change`イベントを購読し、変化時に`setMatches`で更新する。アンマウント時に`removeEventListener`でクリーンアップする。

**依存しているファイル**: なし(ブラウザの`window.matchMedia` APIのみ)

**このファイルを参照しているファイル**: `src/components/Layout/AppLayout.tsx`, `src/components/NetworkMap/NetworkMap.tsx`

---

## `src/components/Layout/AppLayout.tsx`

**役割**: 画面全体のレイアウトシェル。`lg`(1024px)以上は現行どおり「左=NetworkMap、右=EventLog、下=Timeline」の3ペイン固定表示。`lg`未満(Phase2で追加)はマップ/イベントログ/タイムラインの3ペインを同一矩形に絶対配置で重ね、画面下部の`lg:hidden`タブバーで表示を切り替えるモバイル専用レイアウトになる。`WelcomeOverlay`の開閉state(`welcomeOpen`)はここで保持しcontrolled化する(起動ボタンとモーダル本体を構造的に分離するため)。StatsPanelは(Phase1の変更で)NetworkMap側に移動し、本コンポーネントはもう持たない。ルートの高さは`h-[100dvh] lg:h-screen`(モバイルブラウザのアドレスバー伸縮対策として`h-screen`から`h-[100dvh]`に変更したが、Codexレビュー指摘によりこれが`lg`以上にも効いてしまいPhase1の`h-screen`挙動と食い違っていたため、`lg:h-screen`で`lg`以上のみ`100vh`に戻した)。

**主要な型・関数**:
- `MobileTab` type — `'map' | 'log' | 'timeline'`。`lg`未満での選択中タブを表す。
- `tabId(tab)` / `panelId(tab)` (非export) — `MobileTabBar`の`role="tab"`と各ペインの`role="tabpanel"`を`id`/`aria-controls`/`aria-labelledby`で結びつけるための共通ヘルパー(`mobile-tab-${tab}` / `mobile-tabpanel-${tab}`を返す)。
- `mobilePanelAttrs(tab, isDesktop)` (非export) — ペインラッパに付与するARIA属性(`id`/`role="tabpanel"`/`aria-labelledby`/**Codexレビュー2周目の指摘により追加した`tabIndex: 0`**)を返す。`isDesktop`が`true`の場合は空オブジェクトを返し何も付与しない。`lg`以上ではタブバー自体が`lg:hidden`でDOM上も非表示になり対応する`role="tab"`が存在しなくなるため、デスクトップの3ペインレイアウトに`role="tabpanel"`/`tabIndex`のセマンティクスを混入させないための出し分け。`tabIndex: 0`は、操作可能な要素を持たないペイン(イベントログ/タイムライン)でも選択中タブから`Tab`キーで進んだ先にフォーカス先が存在するようにするため。
...
- `MobileTabBar({ activeTab, onSelect })` (非export) — `lg`未満でのみ表示するタブバー(`role="tablist"`、各ボタン`role="tab"`/`aria-selected`/`min-h-11`)。マップ/イベントログ(件数バッジ付き)/タイムライン(件数バッジ付き)の3ボタン。`AppLayout`本体とは別に`useSimulationStore`から`logs.length`/`timeline.length`を購読する専用コンポーネントとして切り出しており、ログ/タイムラインが頻繁に増える攻撃中でも、再レンダーの範囲をこのタブバーだけに限定し`NetworkMap`等の再レンダーを増やさない設計。**Codexレビュー指摘によりARIAのtabパターンを完成**: 各ボタンに`id={tabId(tab.key)}`/`aria-controls={panelId(tab.key)}`を付与し、ロービングフォーカス(選択中のみ`tabIndex=0`、他は`tabIndex=-1`)を実装。`tabRefs`(`useRef<Array<HTMLButtonElement | null>>`)で各ボタンのrefを保持し、`onKeyDown`の`handleKeyDown(event, index)`が`ArrowLeft`/`ArrowRight`のみを処理して隣接タブへ移動(端は反対側へ折り返し)、移動先を`onSelect`で選択状態にしつつ`tabRefs.current[nextIndex]?.focus()`でフォーカスも移す。ルート`div`には**Codexレビュー2周目の指摘により**`order-last`を付与している(理由は下記`AppLayout`の説明を参照)。
- `export default function AppLayout()` — 唯一のexport。Propsなし。`welcomeOpen`(モーダル開閉)と`activeTab`(`lg`未満のタブ選択、既定`'map'`)の2つの`useState`に加え、**Codexレビュー指摘により追加した**`isDesktop = useMediaQuery(DESKTOP_MEDIA_QUERY)`(`../../hooks/useMediaQuery`)を保持する。`isMapActive = isDesktop || activeTab === 'map'`を算出し`<NetworkMap active={isMapActive} />`に渡す(`lg`以上は常に`true`でParticleCanvasの描画を止めない。`lg`未満はmapタブ選択時のみ`true`)。ステージ領域(`relative flex-1 min-h-0 lg:flex lg:flex-col`)の中に「マップ+イベントログの行(`lg`以上は`flex`の横並び、`lg`未満は各ペインが`absolute inset-0`で重なる)」と「タイムライン(`lg`以上は`static`で下段固定、`lg`未満は同じく`absolute inset-0`で重なる)」を配置し、`paneVisibilityClass`で表示を切り替える。各ペインラッパには`{...mobilePanelAttrs(tab, isDesktop)}`をスプレッドする。マップペインには`isolate`を付与し、マップ内のz-indexをこのスコープに閉じ込める。`welcomeOpen`が`false`の間だけマップペイン内に`WelcomeOverlayLauncher`(「?」起動ボタン)を配置し、`WelcomeOverlay`本体はルート直下(`isolate`スコープの外)に`fixed inset-0 z-50`のまま配置する。**Codexレビュー2周目の指摘によりDOM順序を変更**: `MobileTabBar`をステージ領域より**前**のDOM順で配置する(旧: ステージ領域→`MobileTabBar`の順)。これはARIA Tabsパターンで、選択中タブから`Tab`キーを押した際に次のDOM要素である対応パネルへ進めるようにするため(旧順序ではパネル群がタブバーより先にあり前方向の`Tab`でパネルへ移動できなかった)。見た目の並び(タブバーが画面下部)は`MobileTabBar`側の`order-last`で維持しており、`lg`以上は`MobileTabBar`自体が`lg:hidden`でdisplay:noneになるため`order-last`はデスクトップのflex順序に一切影響しない(ステージ領域側は`order`未指定でデフォルト`order:0`のまま)。

**依存しているファイル**: `src/components/NetworkMap/NetworkMap.tsx`, `src/components/EventLog/EventLog.tsx`, `src/components/Timeline/Timeline.tsx`, `src/components/Onboarding/WelcomeOverlay.tsx`(`WelcomeOverlay`, `WelcomeOverlayLauncher`), `src/store/simulationStore.ts`(`useSimulationStore`、`MobileTabBar`内で`logs.length`/`timeline.length`のみ読み取り)、`src/hooks/useMediaQuery.ts`(`useMediaQuery`, `DESKTOP_MEDIA_QUERY`)
...
**主要な型・関数**:
- `NetworkMapProps` interface — `{ active?: boolean }`。**Codexレビュー指摘により追加**。このペインが現在ユーザーに見えているか(`lg`未満のタブ切替でmapタブが非選択の間は`false`)を示す。`ParticleCanvas`の`rAF`ループの起動/停止にのみ使う。省略時は`true`(安全側デフォルト)。
- `NetworkMap({ active = true })` (default export) — ルートコンポーネント。返り値の最上位は`relative flex h-full w-full flex-col overflow-hidden`の縦フレックスで、(1)マップ本体エリア(`ref={containerRef}`、`network-grid-bg relative min-h-0 flex-1 overflow-hidden`)と(2)下部コントロール領域の2段構成(Phase2でモバイル対応のため追加)。**Codexレビュー指摘により**`isDesktop = useMediaQuery(DESKTOP_MEDIA_QUERY)`(`../../hooks/useMediaQuery`)を追加し、CSSの`lg`ブレークポイント(ビューポート幅1024px以上)と完全一致するデスクトップ判定を持つ。`ResizeObserver`は(1)のマップ本体エリアを測定対象にし、`size.width/height`を`useNetworkLayout`(第5引数に`isDesktop`も渡す)に渡す。`nodeVisualScale`(ノード半径/ラベルのスケール、`NodeView`の`scale`propに渡す)は`isDesktop`が`true`の間は常に`1`を固定で使い(コンテナ実寸に依存させない)、`false`の間のみ`computeVisualScale(size.width, size.height)`を使う(コンテナ実寸はビューポート幅と一致しないため、デスクトップ判定にコンテナ幅を使うと`lg`以上でもスケールが1未満になってしまうバグをこの分離で回避している)。`activeAttacks`から`port_scan`の現在ターゲットID集合(`scanTargetIds`)と`ddos`進行中フラグ(`hasDdos`)を算出し、それぞれ`NodeView`の走査枠表示・背景の赤グローに反映する。加えて`ddosAttacks = activeAttacks.filter(a => a.type === 'ddos')`を算出し、SVG内の`AttackEffectsLayer`直後・`NodeView`群の直前に`<BotnetLayer attacks={ddosAttacks} positions={positions} width={size.width} height={size.height} />`を差し込む。SVG内で`<line>`によるリンク描画→`AttackEffectsLayer`→`BotnetLayer`→`NodeView`(`scale={nodeVisualScale}`を渡す)の順にレイヤリングし、その上に`ParticleCanvas`(`active={active}`を渡す)を完全重畳(`position:absolute`)で配置する。マップ本体エリア直下に`<AttackExplanation />`(z-30、攻撃実行中のリアルタイム解説カードをネットワーク図上部に重畳表示。props受け渡しなし)を配置する。既存の`hasDdos`背景グロー・`runDdos`ロジックは変更なし。
...
**依存しているファイル**: `../../store/simulationStore`(`useSimulationStore`、読み取り専用で`nodes`/`links`/`activeAttacks`)、`./useNetworkLayout`、`./ParticleCanvas`、`./NodeView`、`./AttackEffectsLayer`、`./BotnetLayer`、`./AttackToolbar`、`./AttackExplanation`、`../StatsPanel/StatsPanel`、`./constants`(`TRAFFIC_COLORS`, `computeVisualScale`)、`../../hooks/useMediaQuery`(`useMediaQuery`, `DESKTOP_MEDIA_QUERY`)、`./networkMap.css`
...
- `useNetworkLayout(nodes, links, width, height, isDesktop): Record<string, NodePosition>` — `nodes.length`/`width`/`height`/`isDesktop`が変化した時のみ`forceSimulation`を再構築(`useEffect`の依存配列を意図的に絞っている)。**Phase2で`computeLayoutScale`/`computeVisualScale`(`./constants`)によるスケーリングを追加**: `forceLink.distance`は`120 * layoutScale`、`forceManyBody.strength`は`-260 * layoutScale`、`forceCollide`の半径は`NODE_RADIUS[type] * visualScale + 24 * layoutScale`。**Codexレビュー指摘により第5引数`isDesktop`(ビューポート幅ベースのlg判定)を追加**し、`layoutScale`/`visualScale`は`isDesktop`が`true`の間は常に`1`を返す(`false`の間のみ`computeLayoutScale`/`computeVisualScale`を呼ぶ)。旧実装は`width`(マップコンテナの実測幅で、サイドバー等を除くためビューポート幅より狭い)のみで判定していたため、ビューポート1024pxでもコンテナ幅がlgブレークポイント未満になりスケールが1未満になるバグがあった。`tick`イベントでは`margin = NODE_RADIUS[type] * visualScale + 30 * layoutScale`を算出し、`width < margin*2`(または`height < margin*2`)の場合は旧来のclamp式(`min`が先に効いて全ノードが同一座標に潰れるバグがあった)を使わず中央寄せ(`width/2`/`height/2`)にフォールバックしてから`positions`の`state`を更新する。`alphaDecay(0.04)`により収束後は自動的にtickが停止し負荷が限定される。サイズのみが変わった場合は別の`useEffect`で`forceCenter`を更新し`alpha(0.3)`で軽く再加熱する。
...
**主要な型・関数**:
- `ParticleCanvas({ width, height, links, positions, active = true })` (default export) — `links`/`positions`は`ref`に同期させ、`useEffect`内で開始した単一の`rAF`ループから最新値を参照する。各フレームで`devicePixelRatio`に応じてcanvasの実ピクセルサイズを調整し、`clearRect`後に各粒子の`linkId`からリンクのsource/target座標を引いて`source + (target-source)*t`で現在位置を算出、`trafficType`に応じた色(`TRAFFIC_COLORS`)で`shadowBlur`を使ったグロー付きの円を描画する。**Codexレビュー指摘により`active`propを追加**: `useEffect`の依存配列に`active`を加え、`active`が`false`の間は`requestAnimationFrame(render)`を呼ばずループを開始しない(クリーンアップの`cancelAnimationFrame`は常に実行するため、`true`→`false`に変わった時点で確実に停止する)。`active`が再度`true`になるとエフェクトが再実行されループが再開する。粒子自体の状態は`particleSystem`(このコンポーネント外のシングルトン)が保持しているため、停止中も失われず、再開時に自然に描画が続く。
...
- `computeLayoutScale(width, height): number` (Phase2で追加) — `d3-force`の距離/反発力/衝突半径・marginに掛けるスケール。短辺(`Math.min(width, height)`)を基準(640px)に0.5〜1の範囲で縮小する。関数内部に残る`width >= 1024`のガードは、この`width`がマップコンテナの実測幅(ビューポート幅とは一致しない)であるため実質的なデスクトップ判定としては機能しない、念のためのフォールバックである旨を**Codexレビュー指摘によりコメントで明記**。デスクトップ判定(ビューポート幅ベースの`lg`判定)は呼び出し側(`NetworkMap.tsx`/`useNetworkLayout.ts`)が`useMediaQuery(DESKTOP_MEDIA_QUERY)`で行い、`isDesktop`が`true`の間はこの関数自体を呼ばず`1`を固定で使う。
- `computeVisualScale(width, height): number` (Phase2で追加) — ノード半径・ラベルのフォントサイズに掛けるスケール。`computeLayoutScale`と同じ基準だが、読めなくならないよう下限を0.75にクランプする。デスクトップ判定の扱いは`computeLayoutScale`と同様(呼び出し側の`isDesktop`が`true`の間は呼ばれない)。
